...
- **既知の CVE を速く当てること** — 依存の脅威は逆方向に引き合います。速くパッチを当てることは、検証されていないリリースを速く取り込むことでもあります。このアプリにはデプロイ先が無く外部からのリクエストも受けないので、既知の CVE の攻撃面はごく小さい一方、乗っ取られたリリースを install すれば開発機と API キーを失います。そこで新しいリリースには待機期間を置き、CVE は自動マージではなく audit ステップでの報告に留めています。SBOM や署名が無いのはもっと単純な理由で、パッケージとして配布していないからです。
